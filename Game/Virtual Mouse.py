...
mouse_down = False  # Variable to keep track of mouse state
    ...
    if len(lmlist)!=0:
        ...
        if fingers[1] == 1 and fingers[2] == 1:  # If fore finger & middle finger both are up
            ...
            if length < 40:  # If both fingers are really close to each other
                ...
                if not mouse_down:  # If mouse is not already down
                    autopy.mouse.toggle(down=True)  # Press the mouse down
                    mouse_down = True  # Update mouse state
            elif mouse_down:  # If fingers are not close together and mouse is down
                autopy.mouse.toggle(down=False)  # Release the mouse
                mouse_down = False  # Update mouse state
                autopy.key.tap(' ')  # Press space bar to launch rocket
        elif mouse_down:  # If fingers are not close together and mouse is down
            autopy.mouse.toggle(down=False)  # Release the mouse
            mouse_down = False  # Update mouse state
